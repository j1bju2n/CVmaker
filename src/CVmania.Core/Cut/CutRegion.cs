...
public sealed class CutOptions
{
    ...
    /// <summary>
    /// osu!mania scrolls relative to the beatmap's most common BPM. Cutting sections away can change that BPM,
    /// which silently changes the playing speed of every section. True scales all SVs (inserting a green line on
    /// every red line that has none) so that the cut version scrolls at exactly 1.0x in its most common BPM
    /// sections; the mapper's relative SV changes are preserved. False leaves the SVs as they are.
    /// </summary>
    public bool NormalizeScrollSpeed { get; set; } = true;
    /// <summary>True keeps the editor bookmarks that fall inside the kept regions (re-mapped); false drops them all.</summary>
