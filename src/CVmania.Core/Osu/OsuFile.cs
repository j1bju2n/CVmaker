...
public sealed class OsuFile
{
    ...
    /// <summary>[Editor] Bookmarks, in ms, sorted; empty when there are none.</summary>
    public List<int> ReadBookmarks()
    {
        var list = new List<int>();
        var raw = Get("Editor", "Bookmarks");
        if (string.IsNullOrWhiteSpace(raw)) return list;
        foreach (var part in raw.Split(','))
        {
            if (double.TryParse(part.Trim(), NumberStyles.Float, CultureInfo.InvariantCulture, out var b)) list.Add((int)Math.Round(b));
        }
        list.Sort();
        return list;
    }
