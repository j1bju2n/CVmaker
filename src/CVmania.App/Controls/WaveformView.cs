...
namespace CVmania.App.Controls;
...
public sealed class WaveformView : FrameworkElement
{
    ...
    /// <summary>[Editor] bookmarks of the loaded beatmap, ms.</summary>
    public IReadOnlyList<int> Bookmarks { get; set; } = Array.Empty<int>();
    ...
    private static readonly Pen BookmarkPen = Frozen(new Pen(new SolidColorBrush(Color.FromArgb(0xd8, 0x4c, 0xa6, 0xff)), 1.5));
    private static readonly Pen BookmarkOverviewPen = Frozen(new Pen(new SolidColorBrush(Color.FromArgb(0xa0, 0x4c, 0xa6, 0xff)), 1));
    private static readonly Brush BookmarkBrush = Frozen(new SolidColorBrush(Color.FromRgb(0x4c, 0xa6, 0xff)));
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        // bookmarks: one step below red lines in prominence, above the beat grid
        if (ShowBookmarks && Bookmarks.Count > 0)
        {
            foreach (var b in Bookmarks)
            {
                if (b < vs - 1 || b > ve) continue;
                double x = Snap(MsToX(b));
                dc.DrawLine(IsOverview ? BookmarkOverviewPen : BookmarkPen, new Point(x, rulerH), new Point(x, h));
                if (!IsOverview)
                    dc.DrawGeometry(BookmarkBrush, null, Triangle(new Point(x - 5, rulerH), new Point(x + 5, rulerH), new Point(x, rulerH + 7)));
            }
        }
