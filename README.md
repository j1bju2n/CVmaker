# CVmaker — osu! Cut Version Maker

osu! stable에서 **지금 선택한 비트맵**을 실시간으로 읽어 와서, 곡의 파형과 BPM 변속 구간을 보여주고,
...
**English:** CVmaker reads the beatmap currently selected in osu! stable, shows the waveform with its BPM sections, lets you pick regions while listening, and exports a cut version (audio + re-timed `.osu`) whose offset stays exact. Download the zip from Releases, unzip, run `CVmaker.exe`; switch the UI language from the `Language` menu. Drag on the waveform to select, `Enter` to keep, `P` to preview, `Export...` to write the files into your Songs folder (press `F5` in osu!).

## 왜 오프셋이 안 틀어지나
- osu! stable은 `bass.dll`(BASS 2.4)로 오디오를 재생합니다. CVmaker도 **같은 BASS로 디코딩**해서 자르기 때문에 샘플 0번이 게임의 0 ms와 같습니다.
...
5. `내보내기...`를 누르면 설정 창이 뜹니다: 형식(MP3 192k 등), 이음새 크로스페이드, 제목 접미사, 난이도 이름, **제작자 이름(기존 매퍼 이름이 채워져 있고 자유롭게 수정)**, **채보 내용(노트 유지 / 빈 난이도 / 오디오만)**, 출력 폴더, 배경 복사, 오프셋 검증.
...
### 명령줄
`CVmaker.exe "경로\맵.osu"` 로 특정 맵을 바로 열 수 있고(osu! 추적은 꺼짐), `--out "폴더"`로 출력 폴더, `--regions 3000-13000,20000-30000`으로 구간, `--lang ko|en`으로 언어를 미리 지정할 수 있습니다.
`--screenshot "폴더"`는 개발용으로, 창들을 PNG로 저장하고 종료합니다. 파형 아래 노트 표시는 `보기` 메뉴에서 끌 수 있습니다.

## 빌드
```bash
dotnet build CVmaker.sln -c Debug
dotnet test tests/CVmaker.Core.Tests
dotnet run --project src/CVmaker.App
dotnet publish src/CVmaker.App -c Release -r win-x64   # publish/CVmaker.exe 단일 파일 (bass.dll, libmp3lame 포함, 약 67 MB)
```
요구 사항: Windows 10/11 x64, .NET 8 SDK(빌드 시). 배포본은 self-contained라 런타임 설치가 필요 없습니다. 설정은 `%LOCALAPPDATA%\CVmaker\settings.json`에 저장됩니다.

## 구조
```
src/CVmaker.Core   .osu 파서(줄 보존), 타이밍 모델, 컷 플래너(.osu 재작성), 렌더러(구간별 페이드·크로스페이드), 인코더(MP3/OGG/WAV), 오프셋 검증
src/CVmaker.App    WPF UI: osu! 메모리 감시(OsuMemoryDataProvider), BASS 플레이어, 파형/격자/구간 뷰, 내보내기 창, 다국어
